...
export default function KuaishouVideo({ data }: KuaishouVideoProps) {
  ...
  // 下载文件名：kuaishou-作者-标题.mp4（走代理强制保存）
  const safe = (s: string) =>
    s.replace(/[\\/:*?"<>|\r\n]/g, "_").replace(/\.{2,}/g, "_").trim();
  const downloadName = `kuaishou-${safe(kuaishouData.author || "video")}-${safe(
    (kuaishouData.title || "video").slice(0, 30)
  )}.mp4`;
  ...
  const handleDownloadAll = async () => {
    ...
    setDownloading(true);
    try {
      await downloadAllImages(images, `kuaishou-${kuaishouData.author || "图集"}`);
    } finally {
                ...
                <div className="flex items-center gap-2">
                  ...
                  {kuaishouData.authorUrl ? (
                    <a
                      ...
                      title={`打开 ${kuaishouData.author} 的快手主页`}
