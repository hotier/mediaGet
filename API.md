...
**说明**: 自动识别链接所属平台并调用对应解析器，所有平台共用这一个接口。分享链接被好友/他人再次打开时，24 小时内直接命中共享缓存（Cloudflare Cache API，跨实例共享），不会全量重新解析；缓存命中后还会探测主直链，明确死链自动重新解析，避免拿到过期直链。

**参数**（二选一）:
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| url | string | 方式1 | 任意平台分享链接，自动识别平台；**也支持直接粘贴整段分享文案**（如「【标题】…复制打开抖音，看看 https://v.douyin.com/xxx/ …」），服务端自动提取其中链接（提取逻辑与前端共用）。纯文案的专用入口是 `GET /api/parse-text?text=` |
...
- 仅传 `url` 且缺失时返回 `400`，并附 `supportedPlatforms`（支持 ID 解析的平台列表）
...
**说明**: 专门接收 App 复制出的整段分享文案（含标题、引导语），自动提取其中第一个 http(s) 链接后走统一解析流程（平台识别、解析、缓存、直链验证均与 `/api/parse` 一致）。与 `url=` 接口职责分离：`url=` 只接受纯链接（严格校验），分享文案统一走本接口。提取逻辑与前端共用同一份实现（`src/lib/share-text.ts`），保证网页表单与 API 行为一致。
